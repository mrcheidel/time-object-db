...
function timeObjectDB (options) {
  this.basepath = options.basepath || __dirname + "/data/";
...
timeObjectDB.prototype.write =  function (metric, tm, data) {
  return new Promise((resolve, reject) => {
    try {
      ...
      this.updateIndex(metric, _file, fp).then(result => {
...
timeObjectDB.prototype.metricExist = function(metric){
  return fs.existsSync(this.basepath + metric);
}
...
timeObjectDB.prototype.delete = function (_metric){
  result = {};
  return new Promise((resolve, reject) => {
