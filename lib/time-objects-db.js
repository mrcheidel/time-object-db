      ...
      this.getIndexes(metric, fromEpoc, toEpoc).then(data => {
        ...
        Promise.all(ap).then(values => {
          ...
          this.getFirstLast(metric).then(mm => {
            ...
            resolve(result);
            this.events.emit('read', metric, fromEpoc, toEpoc);
          });
        }).catch(reason => {
          result.error = reason;
          reject(result);
    ...
    try {
      ...
      this.updateIndex(metricId, _file, fp).then(result => {
        fs.appendFile(_file, _data, (err) => {
          if (err) reject(err);
          if (action =="I") this.events.emit('insert', metricId, tm, data);
          if (action =="D") this.events.emit('delete', metricId, tm);
          resolve("201");
        });
      }).catch(error => {
        reject(error);
...
timeObjectDB.prototype.clear = function(_metric) {
  ...
  return new Promise((resolve, reject) => {
    ...
    this.events.emit('clear', _metric);
    resolve("204");
  });
}
        ...
        fs.appendFile(
          this.basepath + _metric + "/" + this.indexname, stringify({
            ...
            fs.appendFile(newEntry, stringify({
              ...
              if (err) reject(err);
              this.events.emit('index', newEntry);
              resolve("201");
            });
          }
        );
