  ...
  return new Promise((resolve, reject) => {
    try {
      this.getIndexes(metric,  fromEpoc, toEpoc).then (data => {
        ...
        data.forEach(item => {if (item._tm && item._data);
        ...
        Promise.all(ap).then(values => {
          ...
          result.self.total = bst.tree.getNumberOfKeys();
          result.self.min = bst.tree.getMinKey();
          result.self.max = bst.tree.getMaxKey();
          resolve (result);
...
timeObjectDB.prototype.write = function (metric, tm, data) {
  return new Promise((resolve, reject) => {
    try {
      ...
      this.updateIndex(metric, fp.l1 , _file, fp).then().catch( error => { reject (error);});
...
timeObjectDB.prototype.updateIndex = function (_metric, l1, _file, _data) {
  return new Promise((resolve, reject) => {
    if (!fs.existsSync(_file)) {
      fs.appendFile(this.basepath + _metric + "/" + l1 + "/index.dat", JSON.stringify({"_tm":_data.l3, "_data":_data}) + "\n", (err) => {
        if (err) reject (err);
